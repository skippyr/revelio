...
#define Parse_Data_Type_Option__(option, data_type)                            \
	Parse_Option__(                                                             \
		option,                                                                  \
		if (OPTIONS & is_expecting_path_bit__) { Reveal(last_path); } OPTIONS =  \
			(data_type | is_expecting_path_bit__ |                                \
	       (OPTIONS & non_data_type_bits__));                                   \
		if (is_last_argument__) { Reveal(last_path); } continue;)
#define Parse_Non_Data_Type_Option__(option, action)                           \
	Parse_Option__(                                                             \
		option, if (is_last_argument__) { Reveal(last_path); } action;           \
		continue;)
...
uint8_t Reveal_File(const char* const path)
{
	...
	if (!file)
	{
		return (Throw_Error("can not open file \"", path, "\".",
...
uint8_t Reveal_Directory(const char* const path)
{
	...
	if (!directory)
	{
		return (Throw_Error("can not open directory \"", path, "\".",
		                    "Ensure that you have permissions to read it."));
	}
...
uint8_t Reveal(const char* const path)
{
	...
	if (OPTIONS & is_following_symlinks_bit__ ? stat(path, &metadata)
	                                          : lstat(path, &metadata))
	{
		return (Throw_Error("\"", path, "\" does not exists.",
	...
	default:
		switch (metadata.st_mode & S_IFMT)
		{
			...
			Parse_Return_Case__(
				S_IFLNK,
				Throw_Error("can not reveal contents of symlink \"", path, "\".",
			               "Did you mean to use the option \"--follow-symlinks\" "
			               "before it?"));
		default:
			return (Throw_Error("can not reveal \"", path,
			                    "\" due to its unreadable type.", NULL));
		}
	}
	return (0);
}
