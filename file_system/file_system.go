...
func RevealFile(filePath *string) {
	...
	for scanner.Scan() {
		...
		text := strings.ReplaceAll(scanner.Text(), "\t", strings.Repeat(" ", tabSize))
		graffiti.Print(strings.ReplaceAll(graffiti.EscapePrefixCharacters(text), "\x1b", "@K{white}@F{black}[ESCAPE]@r"))
	}
	graffiti.Println("")
